...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_staff = "SELECT STAFF_NRIC, staff_fname, password FROM staff WHERE email = ?";
    if ($stmt_staff = mysqli_prepare($conn, $sql_staff)) {
        ...
        if (mysqli_stmt_execute($stmt_staff)) {
            ...
            if (mysqli_stmt_num_rows($stmt_staff) == 1) {
                mysqli_stmt_bind_result($stmt_staff, $staff_nric_from_db, $staff_fname_from_db, $hashed_password_from_db);
    ...
    if ($stmt_user = mysqli_prepare($conn, $sql_user)) {
        ...
        if (mysqli_stmt_execute($stmt_user)) {
            ...
            if (mysqli_stmt_num_rows($stmt_user) == 1) {
                mysqli_stmt_bind_result($stmt_user, $id, $first_name, $hashed_password_from_db);
